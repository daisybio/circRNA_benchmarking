...
def plot_corr(total_json_path, polya_json_path, analysis_type):
    ...
    path = f"dumbbell.corr_bar.{analysis_type}.png"
    dumbell_plot(total_data, polya_data, path)
...
def dumbell_plot(total, polya, output_file):
    ...
    polya_scatter = ax.scatter(
        df["polya_r"], df["Tool"], label="polya", s=50,
    ...
    legend_handles = [
        Line2D([0], [0], marker='o', color='w', label='total',
               markerfacecolor=total_color, markeredgecolor='black', markersize=15),
        Line2D([0], [0], marker='o', color='w', label='polya',
               markerfacecolor=polya_color, markeredgecolor='black', markersize=15),
        Line2D([0], [0], color='none', marker='*', markerfacecolor='black',
               markersize=15, label='Significance: * p<0.05 / ** p<0.01 / *** p<0.001')
    ]
...
def _dumbell_plot(total, polya, output_file):
    palette = {
                "total": "#FC8D62",  # orange
                    "polya": "#66C2A5",  # green
               }
    ...
    df = pd.DataFrame(
        {
            ...
            "polya": [polya[k] for k in total.keys()],
        }
    )

    df["mean"] = (df["total"] + df["polya"]) / 2
    df = df.sort_values("Tool", ascending=True)
    ...
    ax.set_title(f"Dumbbell Plot Comparing rRNA Correlation", fontsize=14, fontweight="bold")
    ax.grid(axis="x", linestyle="--", alpha=0.7)
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
...
if __name__ == "__main__":
    ...
    plot_corr(total_json_path, polya_json_path, analysis_type)
