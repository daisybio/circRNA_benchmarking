...
def plot_corr(total_json_path, polya_json_path):
    ...
    out_path = os.path.join(MAIN_DATA_DIR, "dumbbell.corr_bar.png")
    dumbell_plot(total_data, polya_data, out_path)
...
def dumbell_plot(total, polya, output_file):
    ...
    ax.axis('off')
    plt.tight_layout()
    plt.savefig(os.path.join(MAIN_DATA_DIR, "dumbell_legend.png"), dpi=300, bbox_inches='tight')
    plt.close()

if __name__ == "__main__":
    total_json_path = os.path.join(MAIN_DATA_DIR, "total.rna_rrna_corr.json")
    polya_json_path = os.path.join(MAIN_DATA_DIR, "polya.rna_rrna_corr.json")
