...
def run_overlap_coefficient_matrix(tools):
    """
    Computes the Szymkiewicz–Simpson (overlap) coefficient for each pair of tools.
    Returns two DataFrames: NIH and DEEP.
    Each entry (A,B) = |A ∩ B| / min(|A|, |B|), symmetric.
    """
    ...
    for tool1 in tools:
        for tool2 in tools:
            # --- NIH ---
            #a_file = f"/nfs/data3/CIRCEST/runs/test_new_benchmarking/benchmarking_analysis/data_transformations/merge_concatenated_beds/polya/{tool1}_filtered_blacklist.polya.merged.bed"
            #b_file = f"/nfs/data3/CIRCEST/runs/test_new_benchmarking/benchmarking_analysis/data_transformations/merge_concatenated_beds/total/{tool2}_filtered_blacklist.total.merged.bed"
            a_file = f"./../data/GSE138734/merge_concatenated_beds/polya/{tool1}_filtered_blacklist.polya.merged.bed"
            b_file = f"./../data/GSE138734/merge_concatenated_beds/total/{tool2}_filtered_blacklist.total.merged.bed"

            # count intervals
            count_a = int(subprocess.check_output(f"wc -l < {shlex.quote(a_file)}", shell=True).decode().strip())
            count_b = int(subprocess.check_output(f"wc -l < {shlex.quote(b_file)}", shell=True).decode().strip())

            # handle empty files
            if count_a == 0 or count_b == 0:
                nih_df.loc[tool1, tool2] = float("nan")
            else:
                # count number of intervals in A overlapping B
                overlap = int(subprocess.check_output(
                    f"bedtools intersect -a {shlex.quote(a_file)} -b {shlex.quote(b_file)} -u | wc -l",
                    shell=True
                ).decode().strip())
                # Szymkiewicz–Simpson = overlap / min(|A|, |B|)
                nih_df.loc[tool1, tool2] = overlap / min(count_a, count_b)

            # --- DEEP ---
            a_file = f"/nfs/data3/CIRCEST/runs/test_new_benchmarking/benchmarking_analysis/data_transformations_with_blacklist/merge_concatenated_beds/polya/{tool1}_filtered.polya.merged.bed"
            b_file = f"/nfs/data3/CIRCEST/runs/test_new_benchmarking/benchmarking_analysis/data_transformations_with_blacklist/merge_concatenated_beds/total/{tool2}_filtered.total.merged.bed"

            count_a = int(subprocess.check_output(f"wc -l < {shlex.quote(a_file)}", shell=True).decode().strip())
            count_b = int(subprocess.check_output(f"wc -l < {shlex.quote(b_file)}", shell=True).decode().strip())

            if count_a == 0 or count_b == 0:
                deep_df.loc[tool1, tool2] = float("nan")
            else:
                overlap = int(subprocess.check_output(
                    f"bedtools intersect -a {shlex.quote(a_file)} -b {shlex.quote(b_file)} -u | wc -l",
                    shell=True
                ).decode().strip())
                deep_df.loc[tool1, tool2] = overlap / min(count_a, count_b)

    return nih_df, deep_df
...
def run_intersection_matrix(tools):
    # Initialize empty DataFrames (rows=tool1, cols=tool2)
    nih_df = pd.DataFrame(index=tools, columns=tools, dtype=float)
    deep_df = pd.DataFrame(index=tools, columns=tools, dtype=float)
    ...
    for tool1 in tools:
        for tool2 in tools:
            # NIH run
            cmd = f'bash  /nfs/data3/CIRCEST/runs/test_new_benchmarking/benchmarking_analysis/overview_analysis/compare_intersect.sh /nfs/data3/CIRCEST/runs/test_new_benchmarking/benchmarking_analysis/data_transformations/merge_concatenated_beds/polya/{tool1}_filtered_blacklist.polya.merged.bed /nfs/data3/CIRCEST/runs/test_new_benchmarking/benchmarking_analysis/data_transformations/merge_concatenated_beds/total/{tool2}_filtered_blacklist.total.merged.bed'
            proc = subprocess.run(cmd, shell=True, check=True, capture_output=True, text=True)
            ...
            # DEEP run
            cmd = f'bash  /nfs/data3/CIRCEST/runs/test_new_benchmarking/benchmarking_analysis/overview_analysis/compare_intersect.sh /nfs/data3/CIRCEST/runs/test_new_benchmarking/benchmarking_analysis/data_transformations_with_blacklist/merge_concatenated_beds/polya/{tool1}_filtered.polya.merged.bed /nfs/data3/CIRCEST/runs/test_new_benchmarking/benchmarking_analysis/data_transformations_with_blacklist/merge_concatenated_beds/total/{tool2}_filtered.total.merged.bed'
            proc = subprocess.run(cmd, shell=True, check=True, capture_output=True, text=True)
    ...
    for tool1 in tools:
        for tool2 in tools:
            # NIH run
            cmd = f'bash  /nfs/data3/CIRCEST/runs/test_new_benchmarking/benchmarking_analysis/overview_analysis/compare_intersect.sh /nfs/data3/CIRCEST/runs/test_new_benchmarking/benchmarking_analysis/data_transformations/merge_concatenated_beds/polya/{tool1}_filtered_blacklist.polya.merged.bed /nfs/data3/CIRCEST/runs/test_new_benchmarking/benchmarking_analysis/data_transformations/merge_concatenated_beds/polya/{tool2}_filtered_blacklist.polya.merged.bed'
            proc = subprocess.run(cmd, shell=True, check=True, capture_output=True, text=True)
            nih_res = proc.stdout.strip()
            jaccard_val = float(nih_res)  # column 3 = jaccard
            nih_df_poly.loc[tool1, tool2] = jaccard_val
            ...
            # DEEP run
            cmd = f'bash  /nfs/data3/CIRCEST/runs/test_new_benchmarking/benchmarking_analysis/overview_analysis/compare_intersect.sh /nfs/data3/CIRCEST/runs/test_new_benchmarking/benchmarking_analysis/data_transformations_with_blacklist/merge_concatenated_beds/polya/{tool1}_filtered.polya.merged.bed /nfs/data3/CIRCEST/runs/test_new_benchmarking/benchmarking_analysis/data_transformations_with_blacklist/merge_concatenated_beds/polya/{tool2}_filtered.polya.merged.bed'
            proc = subprocess.run(cmd, shell=True, check=True, capture_output=True, text=True)
            deep_res = proc.stdout.strip()
            jaccard_val = float(deep_res)
            deep_df_poly.loc[tool1, tool2] = jaccard_val
    ...
    for tool1 in tools:
        for tool2 in tools:
            # NIH run
            cmd = f'bash  /nfs/data3/CIRCEST/runs/test_new_benchmarking/benchmarking_analysis/overview_analysis/compare_intersect.sh /nfs/data3/CIRCEST/runs/test_new_benchmarking/benchmarking_analysis/data_transformations/merge_concatenated_beds/total/{tool1}_filtered_blacklist.total.merged.bed /nfs/data3/CIRCEST/runs/test_new_benchmarking/benchmarking_analysis/data_transformations/merge_concatenated_beds/total/{tool2}_filtered_blacklist.total.merged.bed'
            proc = subprocess.run(cmd, shell=True, check=True, capture_output=True, text=True)
            nih_res = proc.stdout.strip()
            jaccard_val = float(nih_res)  # column 3 = jaccard
            nih_df_total.loc[tool1, tool2] = jaccard_val
            ...
            # DEEP run
            cmd = f'bash  /nfs/data3/CIRCEST/runs/test_new_benchmarking/benchmarking_analysis/overview_analysis/compare_intersect.sh /nfs/data3/CIRCEST/runs/test_new_benchmarking/benchmarking_analysis/data_transformations_with_blacklist/merge_concatenated_beds/total/{tool1}_filtered.total.merged.bed  /nfs/data3/CIRCEST/runs/test_new_benchmarking/benchmarking_analysis/data_transformations_with_blacklist/merge_concatenated_beds/total/{tool2}_filtered.total.merged.bed'
            proc = subprocess.run(cmd, shell=True, check=True, capture_output=True, text=True)
            deep_res = proc.stdout.strip()
            jaccard_val = float(deep_res)
            deep_df_total.loc[tool1, tool2] = jaccard_val
